...
SDH_data1_no_NA_ <- SDH_data1_no_NA %>%
...
boruta_output <- Boruta(death ~ ., data=na.omit(SDH_data1_no_NA_), doTrace=1)  
...
SDH_data2 <- SDH_data1_no_NA_ %>% dplyr::select(-iv_rehydration,-lethargy, -agecat, -red_eyes, -capillary_refill,-prev_admission, 
                                        -weight_loss , -bulging_fontanelle, -sought_care, -drinking) #-bgluco
...
#computing shapley values using DALEX
explainer_svm <- explain( model = Mortality_svm2, data = X, y = Y,label = "SVM",type = "classification")
...
roc_nn <- roc(test_set_M$death,
               predict(Mortality_nn2, test_set_M, type = "prob")[,1],
               levels = rev(levels(test_set_M$death)))
auc(roc_nn)
ci.auc(roc_nn)
