...
SDH_data <- read_dta("D:/Laptop back_up/IEIP/DATA MANAGEMENT/requests/IPD SIAYA/Data/rota_SCRH_08Dec2020.dta")
...
plot <-aggr(SDH_data1, col=c("green","red"),
            numbers=TRUE,
            sortVars=TRUE)
#Use mice package to impute missing values

SDH_miss <- SDH_data1 %>%
  select(ipdmadmin,ipdpuls,ipdbulgf,ipdreyes,maxvom,ipdsunkf,ipdhu,
         ipdletha,ipdnasal,ipdstrid,ipdmwgt,ipdpneg,ipdmuncons,ipdcaprefi,
         ipdchestin,ipdmfev,nutri_wasting,ipdmdrhd,ipddrink,ipdtfiv,stunting,
         ipdalert,ipdresirit,ipdsunkeye,ipdskinp,ipdtem,ipdhg,ipdmvom)      
...
Mortality_features <- plot(boruta_output, cex.axis=.7, las=2, xlab="", main="Diarrheal Mortality Variable Importance")  

Mortality_features

SDH_data2 <- SDH_data1_no_NA %>% select(-ipddrink, -ipdbulgf, -ipdmwgt, -ipdhu, -ipdmadmin, -ipdcaprefi,
                                        -agecat, -ipdreyes)
